...
class OpenItems:
    ...
    def __init__(self, file_path, bsr_path, output_path):
        self.bsr_lists = None
    ...
    def get_destination_folder(self):
        extension = '.xlsx'
        self.bsr_lists = []
        for file in os.listdir(self.bsr_path):
    ...
    def data_to_destination(self):
        for bsr in self.bsr_lists:
            file = pd.ExcelFile(f'{self.bsr_path}/{bsr}')
            sheet_names = file.sheet_names
            for gl in self.gl_account:
                for sheet in sheet_names:
                    if gl == sheet:
                        data = self.source_file.query(f'GLAccount == {gl}')
                        with pd.ExcelWriter(path=f'{self.output_path}/{bsr}', mode='a', engine='openpyxl',
                                            if_sheet_exists='replace') as writer:
                            data.to_excel(writer, sheet_name=gl, index=False, startrow=1, header=False)
                            print('Completed')
